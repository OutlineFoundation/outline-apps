...
- (id)init {
  ...
  // Initialize packet queue early
  _packetQueue = dispatch_queue_create("com.nthlink.outline.packetqueue", DISPATCH_QUEUE_SERIAL);
  
  // Try to set up file logging with app group, but don't let it crash if it fails
  @try {
    NSString *appGroup = @"group.com.nthlink.outline";
    NSURL *containerUrl = nil;
    
    // Try to get app group container - this may fail if entitlement is missing
    // We catch the error but don't let it crash the extension
    @try {
    ...
    if (containerUrl != nil) {
      logsDirectory = [[containerUrl path] stringByAppendingPathComponent:@"Logs"];
    } else {
      // Fallback: Use the extension's own container directory
      NSArray *paths = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
      NSString *libraryPath = [paths firstObject];
      if (libraryPath) {
        logsDirectory = [libraryPath stringByAppendingPathComponent:@"Logs"];
      } else {
        // Last resort: use a temp directory
        logsDirectory = [NSTemporaryDirectory() stringByAppendingPathComponent:@"VpnExtensionLogs"];
      }
    }
    ...
    BOOL isDir = NO;
    if (![fileManager fileExistsAtPath:logsDirectory isDirectory:&isDir]) {
