...
var outlineDNSResolversV2 = []netip.AddrPort{
...
var linkLocalDNSV2 = netip.MustParseAddrPort("169.254.113.53:53")
...
func NewOutlineDNSTransport(sd transport.StreamDialer, pl transport.PacketListener) (transport.StreamDialer, packetrelay.PacketRelay, func(), error) {
	...
	remoteDNS := outlineDNSResolversV2[rand.IntN(len(outlineDNSResolversV2))]

	// Intercept DNS for StreamDialer: remap TCP connections to linkLocalDNSV2 → remoteDNS.
	sdForward := func(ctx context.Context, addr string) (transport.StreamConn, error) {
		if dst, err := netip.ParseAddrPort(addr); err == nil && dst.Addr().Unmap() == linkLocalDNSV2.Addr() && dst.Port() == linkLocalDNSV2.Port() {
	...
	relayForward := dnsintercept.NewInterceptDNSPacketRelay(dnsListener, baseListener, linkLocalDNSV2, remoteDNS)
	...
	relayTrunc := dnsintercept.NewInterceptDNSPacketRelay(dnsTruncRelay, baseListener, linkLocalDNSV2, remoteDNS)
